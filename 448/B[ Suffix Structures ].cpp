...
<head>
    ...
    <meta name="X-Csrf-Token" content="66590c5185f36bbc730955f452610c47"/>
    ...
    <meta property="og:title" content="Submission #11439655 - Codeforces"/>
    ...
    <title>Submission #11439655 - Codeforces</title>
...
<body><span style='display:none;' class='csrf-token' data-csrf='66590c5185f36bbc730955f452610c47'>&nbsp;</span>
...
<div id="header" style="position: relative;">
    ...
    <div class="lang-chooser">
        ...
        <div >
                        <a href="/enter?back=%2Fcontest%2F448%2Fsubmission%2F11439655">Enter</a>
                     | 
    ...
    <ul class="menu-list main-menu-list">
        ...
        <form method="post" action="/search"><input type='hidden' name='csrf_token' value='66590c5185f36bbc730955f452610c47'/>
...
using namespace std ;
map<char , int > mps , mps1 ;
bool ARR(string s,string s1 ) {
    for( map<char,int>::iterator it= mps1.begin() ; it != mps1.end() ; it++ ) {
        if( mps.find(it->first)!= mps.end() ) {
            if( it->second != mps[it->first] )return 0 ;
        } else return 0 ;
    }
    return mps1.size()==mps.size() ;
}
bool AUTO(string s,string s1 ) {
    int last = 0 ;
    int status[200+1] = {0} ;
    for( int i = 0 ; i < s1.size() ; i++ ) {
        bool f = 1  ;
        for( int j = last ; j < s.size() ; j++ ) {
            if( s[j] == s1[i] && status[j] == 0 ) {
                last = j ;
                f = 0 ;
                status[j] = 1 ;
                break ;
